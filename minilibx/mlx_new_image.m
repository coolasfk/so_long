...
void    *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int height)
{
  ...
  if ((newimg = malloc(sizeof(*newimg))) == NULL)
    return ((void *)0);
  newimg->next = mlx_ptr->imt_glist;
  mlx_ptr->imt_glist = newimg;
  newimg->width = width;
...
mlx_img_ctx_t	*add_img_to_ctx(mlx_imt_glist_t *img, mlx_win_list_t *win)
{
  mlx_img_ctx_t	*imgctx;

  imgctx = win->imt_glist;
  while (imgctx)
    {
      if (imgctx->img == img)
	return (imgctx);
  ...
  imgctx->img = img;
  imgctx->next = win->imt_glist;
  win->imt_glist = imgctx;
...
void    mlx_put_image_to_window(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr, mlx_imt_glist_t *img_ptr, int x, int y)
{
  mlx_img_ctx_t	*imgctx;
...
char    *mlx_get_data_addr(mlx_imt_glist_t *img_ptr, int *bits_per_pixel, int *size_line, int *endian)
{
...
int     mlx_destroy_image(mlx_ptr_t *mlx_ptr, mlx_imt_glist_t *img_todel)
{
  mlx_img_ctx_t	ctx_first;
  mlx_img_ctx_t	*ctx;
  mlx_img_ctx_t	*ctx_to_del;
  mlx_imt_glist_t img_first;
  mlx_imt_glist_t *img;
  mlx_win_list_t *win;

  img_first.next = mlx_ptr->imt_glist;
  img = &img_first;
  while (img && img->next)
    {
  ...
  mlx_ptr->imt_glist = img_first.next;


  win = mlx_ptr->win_list;
  while (win)
    {
      ctx_first.next = win->imt_glist;
      ctx = &ctx_first;
      while (ctx && ctx->next)
	{
