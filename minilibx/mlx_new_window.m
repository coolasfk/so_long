...
- (void) mlx_gl_draw_img:(mlx_imt_glist_t *)img andCtx:(mlx_img_ctx_t *)imgctx andX:(int)x andY:(int)y
{
...
- (void) mlx_gl_draw_font:(mlx_imt_glist_t *)img andCtx:(mlx_img_ctx_t *)imgctx andX:(int)x andY:(int)y andColor:(int)color glyphX:(int)gx glyphY:(int)gy
{
  GLfloat color_tab[4];
  ...
  if (win_to_del->pixmgt)
    {
      ...
      ctx = win_to_del->imt_glist;   // should be null anyway if no pixel management
      while (ctx)
	{
