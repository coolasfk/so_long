...
typedef struct	mlx_imt_glist_s
{
  int			width;
  int			height;
  char			*buffer;
  GLfloat		vertexes[8];
  struct mlx_imt_glist_s	*next;
} mlx_imt_glist_t;
...
typedef struct		mlx_ptr_s
{
  ...
  mlx_imt_glist_t	*imt_glist;
  void			(*loop_hook)(void *);
  void			*loop_hook_data;
  void			*loop_timer;
  mlx_imt_glist_t	*font;
...
int	mlx_destroy_image(mlx_ptr_t *mlx_ptr, mlx_imt_glist_t *img_ptr);
void	*mlx_new_image();
